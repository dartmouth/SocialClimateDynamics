...
rho = 0.25 # Proportion of total population in the rich group, between 0 and 0.5
...
s = 1 # Effectiveness of mitigative action scaling, >= 1

time_delay = 10
tspan = np.linspace(time_delay, 200, 10000)

def heterogeneous_pop_model(Y, t, delay, Tv, Tc, fmax=5, alpha_r=0.5, delta=1, kappa=0.05, alpha_p0=1, xr0=0.9, xp0=0.9, ir0=5, ip0=3.5, rho=0.25, cr=0.4, cp=0.85, kr=1.1, kp=1.35, h=0.5, d=5, w=3, omega=3, dc=1.5, s=1):
    v, xp, xr = Y(t)
    ...
    #x = s*rho*xr+(1-rho)*xp
    l = omega *((1-xr)/(1-xr0))*(ir/ip)*(ip0/ir0)-dc
    #alpha_p = alpha_p0 + d*expit(l)
...
if __name__ == "__main__":
    ...
    test = ddeint(heterogeneous_pop_model, doub_history_function(0.1, 31.5, xr0, xp0), tspan, fargs=(time_delay, 31.5, 2, fmax, alpha_r, delta, kappa, alpha_p0, xr0, xp0, ir0, ip0, rho, cr, cp, kr, kp, h, d, w, omega, dc, s))
